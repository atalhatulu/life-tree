 ...
 *
 * Province identity/plate/region, population2025 and segeRank2025 are reference data.
...
import {SEGE_RANK_2025,SEGE_YEAR} from './sege_2025.js';
...
function calibratedGameProfile(id,scale,population,segeRank){
 ...
 // Small contextual nudge only; SEGE is not an employment or admissions rate.
 const developmentAdjustment=clamp((42-segeRank)/41*.045,-.04,.045);
 return {
  ...
  jobs:round(clamp(base.jobs+populationAdjustment*.65+developmentAdjustment+(hub.jobs??0),.8,1.20)),
  university:round(clamp(base.university+populationAdjustment*.35+developmentAdjustment*.7+(hub.university??0),.82,1.16))
 };
}
...
export const TURKEY_CITIES=Object.freeze(PROVINCES.map(([plate,id,name,region,scale])=>{
 ...
 const segeRank2025=SEGE_RANK_2025[id];
 if(!Number.isSafeInteger(population2025)||population2025<=0)throw new Error('Missing 2025 province population: '+id);
 if(!Number.isInteger(segeRank2025)||segeRank2025<1||segeRank2025>81)throw new Error('Missing 2025 SEGE rank: '+id);
 return Object.freeze({
  id,name,plate,region,regionName:TURKEY_REGIONS[region],scale,
  population2025,populationYear:POPULATION_YEAR,
  segeRank2025,segeYear:SEGE_YEAR,
  ...calibratedGameProfile(id,scale,population2025,segeRank2025),
  ...(LEGACY_BALANCE[id]??{}),
