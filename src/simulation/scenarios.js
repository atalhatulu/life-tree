...
function setScenarioAge(state,age){
 const delta=age-state.player.age;
 state.player.age=age;
 state.year+=delta;
 const family=[
  state.parents?.mother,state.parents?.father,...(state.siblings??[]),
 ...
 for(const person of family)person.age+=delta;
}
