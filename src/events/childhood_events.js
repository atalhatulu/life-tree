...
export const childhoodEvents=[
 {id:'first-family-hobby',title:'Evde Bir Merak',minAge:5,maxAge:9,once:true,condition:s=>Object.keys(s.player.interests).length>0,weight:()=>4,choices:[
  {id:'join',label:'İlgilen',result:'Ailenin sevdiği bir uğraşa sen de ilgi göstermeye başladın.',effect:s=>{const k=Object.keys(s.player.interests)[0];s.player.interests[k]=clamp(s.player.interests[k]+12);s.player.personality.curiosity=growTrait(s.player.personality.curiosity,3);}},
 ...
 {id:'parent-study-pressure',title:'Ders Baskısı',minAge:8,maxAge:15,condition:s=>s.education?.enrolled&&s.household.educationSupport>55,weight:s=>1+s.household.educationSupport/60,choices:[
  {id:'study',label:'Daha çok çalış',result:'Derslerine daha fazla zaman ayırdın.',effect:s=>{s.education.studyEffort=clamp((s.education.studyEffort??0)+25);s.player.personality.discipline=growTrait(s.player.personality.discipline,2);}},
  {id:'balance',label:'Denge kur',result:'Derslerle boş zaman arasında denge kurmaya çalıştın.',effect:s=>{s.education.studyEffort=clamp((s.education.studyEffort??0)+10);s.player.personality.curiosity=growTrait(s.player.personality.curiosity,2);}}
 ]}
];
