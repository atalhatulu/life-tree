...
export function ensureAdultPreferences(state,rng){
 const p=state.player.personality;
 state.preferences??={
  partnershipDesire:clamp(Math.round(30+p.sociability*.42+p.patience*.12+rng.int(-18,18))),
  marriageDesire:clamp(Math.round(30+p.patience*.30+p.ambition*.08+rng.int(-20,20))),
  parenthoodDesire:clamp(Math.round(22+p.patience*.23+p.sociability*.16+rng.int(-28,28))),
